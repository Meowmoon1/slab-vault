...
const SyncConfig = {
  get: () => {
    try { return JSON.parse(localStorage.getItem("slab-vault-sync-config")) || null; } catch { return null; }
  },
  set: (config) => localStorage.setItem("slab-vault-sync-config", JSON.stringify(config)),
  clear: () => localStorage.removeItem("slab-vault-sync-config"),
};

const supaFetch = async (url, apiKey, method = "GET", body = null) => {
  const headers = {
    apikey: apiKey,
...
const SupaSync = {
  pushAll: async (config, slabs) => {
    const { url, apiKey } = config;
    // Upsert all slabs (without images to keep payload small)
    ...
    for (let i = 0; i < rows.length; i += 50) {
      ...
      await supaFetch(`${url}/rest/v1/slabs`, apiKey, "POST", batch);
    }
  },

  pullAll: async (config) => {
    const { url, apiKey } = config;
    const rows = await supaFetch(`${url}/rest/v1/slabs?select=id,data,updated_at&order=updated_at.desc`, apiKey);
    return rows.map((r) => {
      try { return JSON.parse(r.data); } catch { return null; }
    }).filter(Boolean);
  },

  pushOne: async (config, slab) => {
    const { url, apiKey } = config;
    await supaFetch(`${url}/rest/v1/slabs`, apiKey, "POST", [{
      id: slab.id,
  ...
  deleteOne: async (config, id) => {
    const { url, apiKey } = config;
    await supaFetch(`${url}/rest/v1/slabs?id=eq.${id}`, apiKey, "DELETE");
  },

  testConnection: async (config) => {
    const { url, apiKey } = config;
    await supaFetch(`${url}/rest/v1/slabs?select=id&limit=1`, apiKey);
    return true;
  },
};
...
export default function SlabFlipperApp() {
  ...
  // ─── Sync State ───
  const [syncConfig, setSyncConfig] = useState(() => SyncConfig.get());
  ...
  const [lastSync, setLastSync] = useState(() => localStorage.getItem("slab-vault-last-sync") || null);
  ...
  const syncPull = useCallback(async () => {
    ...
    setSyncMessage("Pulling from cloud...");
    try {
      const cloudSlabs = await SupaSync.pullAll(syncConfig);
      setSlabs((local) => {
        // Merge: cloud wins for conflicts, keep local-only entries
        const cloudMap = new Map(cloudSlabs.map((s) => [s.id, s]));
        const localMap = new Map(local.map((s) => [s.id, s]));
        const merged = new Map([...localMap, ...cloudMap]); // cloud overwrites local
        return Array.from(merged.values());
      });
      const now = new Date().toISOString();
      setLastSync(now);
      localStorage.setItem("slab-vault-last-sync", now);
      setSyncStatus("success");
      setSyncMessage(`Synced ${cloudSlabs.length} slabs from cloud`);
    } catch (err) {
      setSyncStatus("error");
  ...
  const syncPush = useCallback(async () => {
    if (!syncConfig) return;
    setSyncStatus("syncing");
    setSyncMessage("Pushing to cloud...");
    try {
      await SupaSync.pushAll(syncConfig, slabs);
      const now = new Date().toISOString();
      setLastSync(now);
      localStorage.setItem("slab-vault-last-sync", now);
      setSyncStatus("success");
      setSyncMessage(`Pushed ${slabs.length} slabs to cloud`);
    } catch (err) {
      setSyncStatus("error");
      setSyncMessage(err.message);
    }
  ...
  const syncFull = useCallback(async () => {
    if (!syncConfig) return;
    setSyncStatus("syncing");
    setSyncMessage("Full sync...");
    try {
      // Push local first, then pull cloud
      await SupaSync.pushAll(syncConfig, slabs);
      const cloudSlabs = await SupaSync.pullAll(syncConfig);
      setSlabs((local) => {
        const cloudMap = new Map(cloudSlabs.map((s) => [s.id, s]));
        const localMap = new Map(local.map((s) => [s.id, s]));
        const merged = new Map([...localMap, ...cloudMap]);
        return Array.from(merged.values());
      });
      const now = new Date().toISOString();
      setLastSync(now);
      localStorage.setItem("slab-vault-last-sync", now);
      setSyncStatus("success");
      setSyncMessage("Full sync complete!");
    } catch (err) {
      setSyncStatus("error");
      setSyncMessage(err.message);
    }
  ...
  const saveSlab = useCallback((slab) => {
    const savedSlab = editId ? { ...slab, id: editId } : { ...slab, id: uid() };
    if (editId) {
      setSlabs((prev) => prev.map((s) => (s.id === editId ? savedSlab : s)));
    } else {
      setSlabs((prev) => [...prev, savedSlab]);
    }
    // Background sync
    if (syncConfig) {
      SupaSync.pushOne(syncConfig, savedSlab).catch(() => {});
    }
    setShowForm(false);
    setEditId(null);
  }, [editId, syncConfig]);
  ...
  const deleteSlab = useCallback((id) => {
    ...
    if (syncConfig) {
      SupaSync.deleteOne(syncConfig, id).catch(() => {});
    }
    setViewId(null);
  }, [syncConfig]);
  ...
  const handleTestAndSave = useCallback(async () => {
    const url = settingsUrl.trim().replace(/\/$/, "");
    const key = settingsKey.trim();
    if (!url || !key) { alert("Please enter both URL and API key."); return; }
    setSettingsTesting(true);
    try {
      const config = { url, apiKey: key };
      ...
      setSettingsUrl("");
      setSettingsKey("");
      alert("Connected! Your data will now sync across devices.");
      // Initial push
      await SupaSync.pushAll(config, slabs);
      const now = new Date().toISOString();
      setLastSync(now);
      localStorage.setItem("slab-vault-last-sync", now);
    } catch (err) {
      alert("Connection failed: " + err.message + "\n\nMake sure the table 'slabs' exists and your API key is correct.");
    }
    setSettingsTesting(false);
  }, [settingsUrl, settingsKey, slabs]);

  const handleDisconnectSync = useCallback(() => {
    if (window.confirm("Disconnect cloud sync? Your local data will be kept.")) {
      SyncConfig.clear();
      setSyncConfig(null);
      setLastSync(null);
      localStorage.removeItem("slab-vault-last-sync");
      setShowSettings(false);
    }
  }, []);
  ...
  const exportData = useCallback(() => {
    const data = {
      ...
      slabs: slabs.map(({ frontImage, backImage, ...rest }) => ({
        ...rest,
        frontImage: frontImage ? "[image]" : null,
        backImage: backImage ? "[image]" : null,
      })),
    };
    ...
    a.href = url;
    a.download = `slab-vault-backup-${new Date().toISOString().split("T")[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, [slabs]);

  const exportDataFull = useCallback(() => {
    const data = {
      version: 1,
      exportDate: new Date().toISOString(),
      slabCount: slabs.length,
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `slab-vault-full-backup-${new Date().toISOString().split("T")[0]}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, [slabs]);
    ...
    reader.onload = (evt) => {
      try {
        ...
        if (!data.slabs || !Array.isArray(data.slabs)) {
          alert("Invalid backup file — no slabs found.");
          return;
        }
        const action = window.confirm(
          `Found ${data.slabs.length} slabs from ${data.exportDate || "unknown date"}.\n\nOK = Replace all current data\nCancel = Merge with existing data`
        );
        if (action) {
          setSlabs(data.slabs);
        } else {
          setSlabs((prev) => {
            ...
            const newSlabs = data.slabs.filter((s) => !existingIds.has(s.id));
            return [...prev, ...newSlabs];
          });
        }
        alert(`Import complete! ${action ? "Replaced" : "Merged"} ${data.slabs.length} slabs.`);
      } catch (err) {
  ...
  const clearAllData = useCallback(() => {
    if (window.confirm("Are you sure you want to delete ALL slab data? This cannot be undone.")) {
      if (window.confirm("Really? This will permanently delete everything.")) {
        setSlabs([]);
      }
    }
  }, []);
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 11, color: C.textDim, textAlign: "right", display: "flex", alignItems: "center", gap: 12 }}>
              <span><span style={{ color: C.accent }}>{slabs.length}</span> slabs tracked</span>
              <div style={{ position: "relative" }}>
                <button onClick={() => setShowDataMenu((p) => !p)} style={{
                  background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8,
                  padding: "6px 10px", cursor: "pointer", color: C.textMuted, fontSize: 12,
                ...
                {showDataMenu && (
                  <>
                    <div onClick={() => setShowDataMenu(false)} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 199 }} />
                    <div style={{
                      position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 200,
                      background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10,
                      padding: 8, minWidth: 220, boxShadow: "0 12px 40px rgba(0,0,0,0.5)",
                    }}>
                      <div style={{ padding: "6px 10px", fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: "0.1em" }}>
                        Backup & Transfer
                      </div>
                      <button onClick={() => { exportData(); setShowDataMenu(false); }} style={{
                        width: "100%", textAlign: "left", background: "none", border: "none",
                        padding: "10px 10px", color: C.text, fontSize: 12, fontFamily: "inherit",
                        cursor: "pointer", borderRadius: 6, display: "flex", alignItems: "center", gap: 8,
                      }}
                      onMouseEnter={(e) => e.currentTarget.style.background = C.surface2}
                      onMouseLeave={(e) => e.currentTarget.style.background = "none"}
                      >
                        {Icons.download} <div><div style={{ fontWeight: 600 }}>Export (Light)</div><div style={{ fontSize: 10, color: C.textDim, marginTop: 2 }}>Without images · Small file</div></div>
                      </button>
                      <button onClick={() => { exportDataFull(); setShowDataMenu(false); }} style={{
                        width: "100%", textAlign: "left", background: "none", border: "none",
                        padding: "10px 10px", color: C.text, fontSize: 12, fontFamily: "inherit",
                        cursor: "pointer", borderRadius: 6, display: "flex", alignItems: "center", gap: 8,
                      }}
                      ...
                      onMouseLeave={(e) => e.currentTarget.style.background = "none"}
                      >
                        {Icons.download} <div><div style={{ fontWeight: 600 }}>Export (Full)</div><div style={{ fontSize: 10, color: C.textDim, marginTop: 2 }}>With images · Larger file</div></div>
                      </button>
                      <div style={{ height: 1, background: C.border, margin: "4px 0" }} />
                      <button onClick={() => { importFileRef.current?.click(); setShowDataMenu(false); }} style={{
                        width: "100%", textAlign: "left", background: "none", border: "none",
                        padding: "10px 10px", color: C.text, fontSize: 12, fontFamily: "inherit",
                        cursor: "pointer", borderRadius: 6, display: "flex", alignItems: "center", gap: 8,
                      }}
                      ...
                      >
                        ...
                        <div><div style={{ fontWeight: 600 }}>Import Backup</div><div style={{ fontSize: 10, color: C.textDim, marginTop: 2 }}>Replace or merge from file</div></div>
                      </button>
                      <div style={{ height: 1, background: C.border, margin: "4px 0" }} />
                      <button onClick={() => { clearAllData(); setShowDataMenu(false); }} style={{
                        width: "100%", textAlign: "left", background: "none", border: "none",
                        padding: "10px 10px", color: C.red, fontSize: 12, fontFamily: "inherit",
                        cursor: "pointer", borderRadius: 6, display: "flex", alignItems: "center", gap: 8,
                      }}
                      onMouseEnter={(e) => e.currentTarget.style.background = C.redDim}
                      onMouseLeave={(e) => e.currentTarget.style.background = "none"}
                      >
                        {Icons.trash} <div><div style={{ fontWeight: 600 }}>Clear All Data</div><div style={{ fontSize: 10, color: C.textDim, marginTop: 2 }}>Delete everything</div></div>
                      </button>
                    </div>
                  </>
                )}
              ...
              {/* Sync Button */}
              {syncConfig && (
                <button onClick={syncFull} disabled={syncStatus === "syncing"} style={{
                  background: syncStatus === "success" ? C.greenDim : syncStatus === "error" ? C.redDim : C.surface2,
                  border: `1px solid ${syncStatus === "success" ? C.green : syncStatus === "error" ? C.red : C.border}`,
                  borderRadius: 8, padding: "6px 10px", cursor: syncStatus === "syncing" ? "wait" : "pointer",
                  color: syncStatus === "success" ? C.green : syncStatus === "error" ? C.red : C.textMuted,
                  fontSize: 12, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5, transition: "all 0.2s",
                }}>
              ...
              {/* Settings Gear */}
              <button onClick={() => { setShowSettings(true); if (syncConfig) { setSettingsUrl(syncConfig.url); setSettingsKey(syncConfig.apiKey); } }} style={{
                background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8,
                padding: "6px 8px", cursor: "pointer", color: C.textMuted, fontSize: 12,
                fontFamily: "inherit", display: "flex", alignItems: "center", transition: "border-color 0.2s",
              }}
              onMouseEnter={(e) => e.currentTarget.style.borderColor = C.accent}
              onMouseLeave={(e) => e.currentTarget.style.borderColor = C.border}
              >
      ...
      {syncStatus !== "idle" && (
        <div style={{
          ...
          background: syncStatus === "success" ? C.green : syncStatus === "error" ? C.red : C.accent,
          color: "#0a0a0f", padding: "8px 20px", borderRadius: 20, fontSize: 12, fontWeight: 600,
          fontFamily: "inherit", boxShadow: "0 4px 20px rgba(0,0,0,0.4)", whiteSpace: "nowrap",
        }}>
          {syncMessage}
        </div>
      )}
      ...
      {/* Settings Modal */}
      {showSettings && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 250, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={() => setShowSettings(false)} style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)" }} />
          <div style={{
            position: "relative", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16,
            padding: 28, width: "90%", maxWidth: 480, maxHeight: "80vh", overflowY: "auto",
            boxShadow: "0 20px 60px rgba(0,0,0,0.5)",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h2 style={{ margin: 0, fontSize: 18, fontFamily: "'Playfair Display', Georgia, serif", color: C.accent }}>
                Cloud Sync Settings
              </h2>
              <button onClick={() => setShowSettings(false)} style={{
                background: "none", border: "none", color: C.textMuted, cursor: "pointer", fontSize: 20,
              }}>{Icons.x}</button>
            </div>

            {syncConfig ? (
              <div>
                <div style={{ background: C.greenDim, border: `1px solid ${C.green}`, borderRadius: 10, padding: 14, marginBottom: 16 }}>
                  <div style={{ color: C.green, fontSize: 13, fontWeight: 600, marginBottom: 4 }}>✓ Connected</div>
                  <div style={{ color: C.textMuted, fontSize: 11, wordBreak: "break-all" }}>{syncConfig.url}</div>
                  {lastSync && <div style={{ color: C.textDim, fontSize: 10, marginTop: 6 }}>Last sync: {new Date(lastSync).toLocaleString()}</div>}
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button onClick={() => { syncFull(); setShowSettings(false); }} style={{
                    flex: 1, padding: "10px", background: C.accent, color: "#0a0a0f", border: "none",
                    borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 13, fontFamily: "inherit",
                  }}>Full Sync Now</button>
                  <button onClick={() => { syncPush(); setShowSettings(false); }} style={{
                    flex: 1, padding: "10px", background: C.surface2, color: C.text, border: `1px solid ${C.border}`,
                    borderRadius: 8, cursor: "pointer", fontWeight: 500, fontSize: 13, fontFamily: "inherit",
                  }}>Push to Cloud</button>
                  <button onClick={() => { syncPull(); setShowSettings(false); }} style={{
                    flex: 1, padding: "10px", background: C.surface2, color: C.text, border: `1px solid ${C.border}`,
                    borderRadius: 8, cursor: "pointer", fontWeight: 500, fontSize: 13, fontFamily: "inherit",
                  }}>Pull from Cloud</button>
                </div>
                <button onClick={handleDisconnectSync} style={{
                  width: "100%", marginTop: 12, padding: "10px", background: "none", color: C.red,
                  border: `1px solid ${C.red}`, borderRadius: 8, cursor: "pointer", fontSize: 12, fontFamily: "inherit",
                }}>Disconnect Sync</button>
              </div>
            ) : (
              <div>
                <p style={{ color: C.textMuted, fontSize: 12, lineHeight: 1.6, margin: "0 0 16px" }}>
                  Connect to Supabase to sync your slabs across devices. Follow these steps:
                </p>
                <ol style={{ color: C.textMuted, fontSize: 12, lineHeight: 2, paddingLeft: 20, margin: "0 0 16px" }}>
                  <li>Go to <span style={{ color: C.accent }}>supabase.com</span> and create a free account</li>
                  <li>Create a new project (any name, any region)</li>
                  <li>Go to <span style={{ color: C.accent }}>SQL Editor</span> and run this query:</li>
                </ol>
                <div style={{
                  background: C.bg, borderRadius: 8, padding: 12, marginBottom: 16,
                  fontSize: 11, color: C.green, fontFamily: "monospace", overflowX: "auto", whiteSpace: "pre",
                  border: `1px solid ${C.border}`, cursor: "pointer", position: "relative",
                }} onClick={() => {
                  navigator.clipboard?.writeText("create table slabs (\n  id text primary key,\n  data jsonb not null,\n  updated_at timestamptz default now()\n);");
...
{`create table slabs (
  ...
  updated_at timestamptz default now()
                  ...
                  <div style={{ position: "absolute", top: 6, right: 8, fontSize: 9, color: C.textDim }}>click to copy</div>
                </div>
                <ol start={4} style={{ color: C.textMuted, fontSize: 12, lineHeight: 2, paddingLeft: 20, margin: "0 0 16px" }}>
                  <li>Go to <span style={{ color: C.accent }}>Settings → API</span></li>
                  <li>Copy your <strong>Project URL</strong> and <strong>anon public key</strong></li>
                </ol>
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, color: C.textDim, display: "block", marginBottom: 4 }}>Project URL</label>
                  <input value={settingsUrl} onChange={(e) => setSettingsUrl(e.target.value)}
                    placeholder="https://xxxxx.supabase.co" style={{
                    width: "100%", padding: "10px 12px", background: C.bg, border: `1px solid ${C.border}`,
                    borderRadius: 8, color: C.text, fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box",
                  }} />
                </div>
                <div style={{ marginBottom: 16 }}>
                  <label style={{ fontSize: 11, color: C.textDim, display: "block", marginBottom: 4 }}>Anon Public Key</label>
                  <input value={settingsKey} onChange={(e) => setSettingsKey(e.target.value)}
                    placeholder="eyJhbGciOiJIUzI1NiIs..." style={{
                    width: "100%", padding: "10px 12px", background: C.bg, border: `1px solid ${C.border}`,
                    borderRadius: 8, color: C.text, fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box",
                  }} />
                </div>
                <button onClick={handleTestAndSave} disabled={settingsTesting} style={{
                  width: "100%", padding: "12px", background: C.accent, color: "#0a0a0f", border: "none",
                  borderRadius: 8, cursor: settingsTesting ? "wait" : "pointer", fontWeight: 700, fontSize: 14, fontFamily: "inherit",
                  opacity: settingsTesting ? 0.7 : 1,
                }}>
                  {settingsTesting ? "Testing connection..." : "Connect & Sync"}
                </button>
              </div>
            )}
          </div>
...
function SlabList({ slabs, searchTerm, onSearch, filterCompany, onFilterCompany, filterStatus, onFilterStatus, filterEra, onFilterEra, onAdd, onView, onEdit, onDelete }) {
  ...
  const activeSlabs = slabs.filter((s) => s.status !== "Sold" && s.status !== "Shipped");
  const completedSlabs = slabs.filter((s) => s.status === "Sold" || s.status === "Shipped");
  const displaySlabs = subTab === "active" ? activeSlabs : subTab === "completed" ? completedSlabs : slabs;

  const subTabStyle = (t) => ({
    padding: "8px 16px", background: subTab === t ? C.surface2 : "transparent",
    color: subTab === t ? C.text : C.textDim, border: subTab === t ? `1px solid ${C.border}` : "1px solid transparent",
    borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: subTab === t ? 600 : 400,
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 16 }}>
        <button onClick={() => setSubTab("active")} style={subTabStyle("active")}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.green, display: "inline-block" }} />
          Active <span style={{ color: C.textDim, fontWeight: 400 }}>({activeSlabs.length})</span>
        </button>
        <button onClick={() => setSubTab("completed")} style={subTabStyle("completed")}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: C.accent, display: "inline-block" }} />
          Completed <span style={{ color: C.textDim, fontWeight: 400 }}>({completedSlabs.length})</span>
        </button>
        <button onClick={() => setSubTab("all")} style={subTabStyle("all")}>
          All <span style={{ color: C.textDim, fontWeight: 400 }}>({slabs.length})</span>
        </button>
      </div>
      ...
      {displaySlabs.length === 0 ? (
        <EmptyState
          icon={subTab === "completed" ? "✓" : "◆"}
          title={subTab === "completed" ? "No completed flips" : subTab === "active" ? "No active slabs" : "No slabs yet"}
          subtitle={subTab === "completed" ? "Slabs marked as Sold or Shipped will appear here." : subTab === "active" ? "Add slabs to start tracking your collection." : "Start building your vault by adding your first slab."}
          action={subTab !== "completed" ? <Btn onClick={onAdd}>{Icons.plus} Add Your First Slab</Btn> : null}
        />
      ) : (
        <div style={{ display: "grid", gap: 8 }}>
          ...
          <div style={{
            display: "grid",
            gridTemplateColumns: subTab === "completed"
              ? "2.5fr 1.2fr 0.8fr 1fr 1fr 0.8fr 0.8fr 0.5fr"
              : "2.5fr 1.2fr 1fr 1fr 1fr 0.8fr 0.5fr",
            gap: 12, padding: "8px 16px", fontSize: 10, color: C.textDim,
            textTransform: "uppercase", letterSpacing: "0.1em",
          }}>
            <span>Card</span><span>Grade</span>
            {subTab === "completed" && <span>Era</span>}
            <span>Buy</span><span>Sell</span><span>Profit</span><span>Held</span><span></span>
          </div>
          {displaySlabs.map((s) => {
            const flip = calcFlip(s);
            const isSold = s.status === "Sold" || s.status === "Shipped";
            return (
              <div key={s.id} onClick={() => onView(s.id)} style={{
                ...
                fontSize: 13, opacity: isSold && subTab === "all" ? 0.6 : 1,
              }}
              ...
              >
                <div>
                  <div style={{ fontWeight: 600, marginBottom: 2, display: "flex", alignItems: "center", gap: 6 }}>
                    {s.cardName}
                    {isSold && subTab === "all" && <span style={{ fontSize: 9, background: C.accentGlow, color: C.accent, padding: "2px 6px", borderRadius: 4 }}>SOLD</span>}
                  </div>
                  <div style={{ fontSize: 11, color: C.textDim }}>
                    {s.set} {s.year && `(${s.year})`}{s.certNumber ? ` · #${s.certNumber}` : ""}
                    {subTab !== "completed" && s.status === "Listed" && <span style={{ color: C.blue, marginLeft: 6 }}>● Listed</span>}
                  </div>
                </div>
                <div><Badge color={gradeColor(s.gradingCompany)}>{s.gradingCompany} {s.grade}</Badge></div>
                {subTab === "completed" && <div style={{ fontSize: 11, color: C.textMuted }}>{s.era?.split("(")[0]?.trim().slice(0,10)}</div>}
                <div>{fmtCurrency(s.buyPrice)}</div>
                <div>{s.sellPrice ? fmtCurrency(s.sellPrice) : <span style={{ color: C.textDim }}>—</span>}</div>
                <div style={{ color: flip.netProfit >= 0 ? C.green : C.red, fontWeight: 600 }}>
                  {isSold ? fmtCurrency(flip.netProfit) : <span style={{ color: C.textDim }}>—</span>}
                </div>
